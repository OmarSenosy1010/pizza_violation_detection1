# """
# detection.py
# Consumes frames from RabbitMQ, detects hands/scoopers/pizza using YOLO, tracks hands, and publishes violation frames if a hand touches pizza without a scooper.
# """
# import cv2
# import pika
# import base64
...
#     """
#     Register a new hand tracker with its centroid.
#     """
#     global next_id
#     trackers[next_id] = {
#         "centroid": centroid,
...
#     """
#     Update hand trackers with new detections (centroids).
#     Handles registering new hands and removing lost ones.
#     """
#     global trackers, next_id
...
#     # Increment disappear count for unmatched trackers
...
#     """
#     Callback for RabbitMQ: processes a frame, detects objects, updates trackers, and checks for violations.
#     """
...
#             # If scooper seen near hand in ROI
#             if any(x < cx < x+w and y < cy < y+h for (x, y, w, h) in roi_list):
#                 if any(abs(cx - sx) < 20 and abs(cy - sy) < 20 for (sx, sy) in scoopers):
#                     tr["scooper_seen"] = True
#             # If hand enters pizza area
...
#     # If violation detected, publish frame
#     if violation:
#         _, buf  = cv2.imencode('.jpg', frame)
#         img_out = base64.b64encode(buf).decode('utf-8')
#         msg     = {"frame_id": frame_id, "image": img_out}
#         ch.basic_publish(
#             exchange='', routing_key=DETECT_QUEUE,
#             body=json.dumps(msg),
...
def callback(ch, method, properties, body):
    ...
    for box in results.boxes:
        ...
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
        cv2.putText(frame, cls, (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
    ...
    # Always send frame (with or without violation)
    _, buf = cv2.imencode('.jpg', frame)
    img_out = base64.b64encode(buf).decode('utf-8')
    msg = {
        "frame_id": frame_id,
    ...
    if violation:
        ...
        print(f"[OK] Frame {frame_id} processed (no violation)")
